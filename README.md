...
- /api/status : 디바이스 상태 JSON (버전, IP, RSSI, free heap, uptime, WS 통계)
- /scan : WiFi 스캔 시작
- /wifistatus : 스캔 결과 JSON
- /wifireset : 저장된 WiFi 삭제
- /connect2ssid : SSID/비밀번호 저장 (JSON body)
- /savepreset : 프리셋 저장 (JSON body)
- /preset.ini : 프리셋 파일
...
- Serial baud: 9600 (RS485 라인과 공유, 디버그 출력은 `RS485_CLEAN_BUS=1`로 차단)
- ENABLE_WEBSOCKET = 1
- ENABLE_WEBPAGE = 1
- WS 연결 시 Hello 메시지 전송 (ping은 안정성 이유로 비활성)
- WS stats 로그 출력 간격: 5초 (WS 송신 byte/frame, dropped, free heap 포함)
- Serial 수신 버퍼: HW UART RX 1KB + 누적 패킷 버퍼 8KB
- WS → RS485 fragment 재조립 버퍼: 8KB
- 패킷 타임아웃 (idle gap): 5ms (`PACK_TIMEOUT_US`)
- WS heap 가드: free heap < 8KB 시 송신 drop (`WS_HEAP_GUARD`)
